...
def get_special_progress_keyboard():
    # В режиме «Особенная настройка» поддержки нет.
    # Кнопка «Обновить» позволяет вручную проверить актуальный
    # прогресс invite после того, как новые пользователи вошли
    # по реферальной ссылке и подтвердили подписку.
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔄 Обновить",
                    callback_data="special_refresh"
                )
            ],
            [
                InlineKeyboardButton(
...
async def confirm_referral(user_id: int):
    ...
    # Если у реферера активна специальная заявка,
...
@dp.callback_query(F.data == "special_refresh")
async def process_special_refresh(callback: CallbackQuery):
    """Обновляет прогресс специальной заявки без повторного ввода данных."""
    user_id = callback.from_user.id

    cursor.execute(
        """
        SELECT special_active, special_username, special_model,
    ...
    special_active, username, model, start_invites, special_completed = row

    if special_completed:
        await callback.answer("✅ Заявка уже выполнена.", show_alert=True)
        return

    if not special_active:
        await callback.answer("❌ Сначала начните особенную настройку.", show_alert=True)
        return

    cursor.execute(
        "SELECT invites_count FROM users WHERE user_id = ?",
        (user_id,)
    )
    invite_row = cursor.fetchone()
    current_invites = invite_row[0] if invite_row else 0
    start_invites = start_invites or 0
    progress = min(5, max(0, current_invites - start_invites))
    remaining = max(0, 5 - progress)

    if progress >= 5 and model:
        completed = await complete_special_request(user_id)
        if completed:
            await callback.answer("✅ 5/5! Заявка отправлена.")
            return
    ...
    if model:
        text = (
            f"⚙️ Особенная настройка\n\n"
            f"📱 Модель: {model}\n"
            f"📩 Настройка будет отправлена на ваш TG: {username}\n\n"
            f"👥 Invite: {progress}/5\n"
            f"🏆 Осталось пригласить: {remaining}\n\n"
            f"🔗 Ваша реферальная ссылка:\n{ref_link}\n\n"
            "Нажмите «🔄 Обновить» после того, как приглашённые "
            "пользователи зайдут по вашей ссылке и подпишутся на все 3 канала."
        )
    else:
        text = (
            f"⚙️ Особенная настройка\n\n"
